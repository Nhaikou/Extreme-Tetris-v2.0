...
bool BlockI::spawn()
{
	...
	{
		if (board->grid[board->getSpawnPoint().x + i][board->getSpawnPoint().y].getTexture() == &board->emptyTex)
		{
...
void BlockI::rotate(int direction)
{
	...
	if (rotation == 1 && positions[1].x > 0 && positions[1].x + 2 < board->getSize().x &&
		...
		board->grid[positions[3].x + 2][positions[3].y + 2].getTexture() == &board->emptyTex)
	{
