...
bool BlockI::spawn()
{
	...
	{
		if (board->grid[board->getSpawnPoint().x + i][board->getSpawnPoint().y] == BlockType::EMPTY)
		{
			positions[i] = sf::Vector2i(board->getSpawnPoint().x + i, board->getSpawnPoint().y);
			board->grid[positions[i].x][positions[i].y] = BlockType::BLOCKI;
		}
		else
		{
			return false;
...
void BlockI::rotate(int direction)
{
	...
	if (rotation == 1 && positions[1].x > 0 && positions[1].x + 2 < board->getSize().x &&
		...
		board->grid[positions[3].x + 2][positions[3].y + 2] == BlockType::EMPTY)
	{
